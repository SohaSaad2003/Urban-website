...
# Configure Gemini API for 6th October
genai.configure(api_key="AIzaSyAvUwL0J7o1CGSSa9R4XcIcFTCuE6JAQTo")

# Load GeoJSON data for each city
try:
...
# System instructions for each city
october_instruction ="""
...
✅ مهامك:
1. لرد على استفسارات المستخدمين بدقة وسرعة، دون مطالبتهم بمعلومات إضافية مثل الإحداثيات أو تحديدات دقيقة.
2.إذا سأل المستخدم عن منطقة معينة ، استخدم أقرب بيانات متوفرة في قاعدة بياناتك لتقدير النتائج، حتى لو لم تكن تملك تقسيمًا دقيقًا للمدينة. لا تخبر المستخدم بعدم توفر بيانات، بل اعرض عليه أقرب النتائج الممكنة بناءً على المعلومات المتوفرة لديك.
...
◾ عندما تقترح أماكن أو خدمات:
- اعرض خدمات مختلفة من أماكن متعددة داخل المدينة، ولا تكرر نفس المكان (مثل "مول العاشر" أو "سنتر المدينة") إلا إذا كان فعلاً هو الأقرب.
- استخرج مجموعة متنوعة من الأماكن بناء على بيانات خطوط الطول والعرض المتاحة لديك.
- غطِّ مناطق سكنية وتجارية مختلفة عند تقديم الاقتراحات، مثل الحي الأول، الحي الثالث، المجاورات، أو الطرق الرئيسية.
- هدفك هو تقديم تجربة ثرية للمستخدم بدون تكرار أو تعميم زائد.
...
october_model = genai.GenerativeModel(
    ...
    system_instruction=october_instruction
)

ramadan_model = genai.GenerativeModel(
    model_name="gemini-1.5-pro-latest",
    generation_config=generation_config,
    system_instruction=ramadan_instruction
)

# Initialize chat sessions
october_chat = october_model.start_chat(history=[])
ramadan_chat = ramadan_model.start_chat(history=[])
...
def recommend_services_by_area(city_data):
...
def analyze_city(city_name, city_data):
...
def chat_with_gemini_6th_october(user_query):
    ...
    if "حلل المدينة" in user_query or "المناطق الناقصة" in user_query:
        return analyze_city("السادس من أكتوبر", october_data)

    october_chat.send_message(user_query)
    return october_chat.last.text
...
def chat_with_gemini_10th_ramadan(user_query):
    ...
    if "حلل المدينة" in user_query or "المناطق الناقصة" in user_query:
        return analyze_city("العاشر من رمضان", ramadan_data)

    ramadan_chat.send_message(user_query)
    return ramadan_chat.last.text
