...
def classify_regions_and_generate_ndvi(image_path):
    ...
    try:
        ...
        plt.axis('off')
        plt.savefig(os.path.join(current_app.root_path, classified_image_path), dpi=300)
        plt.close()
        ...
        plt.axis('off')
        plt.savefig(os.path.join(current_app.root_path, ndvi_map_path), dpi=300)
        plt.close()
        ...
        return {
            'success': True,
            'classified_image': classified_image_path,
            'ndvi_map': ndvi_map_path,
...
def process_and_analyze_image(image_path):
    ...
    try:
        ...
        # Return processed results with the calculated percentages
        return {
            'success': True,
